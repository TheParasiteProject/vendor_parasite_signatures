...
function write_product_certificate_overrides() {
	...
	echo 'ifeq ($(TARGET_SUPPORTS_PREBUILT_UPDATABLE_APEX), false)' >>"$PRODUCTMK"
	echo 'PRODUCT_MAINLINE_SEPOLICY_DEV_CERTIFICATES=$(CERTIFICATE_COMMON)/data' >>"$PRODUCTMK"
	echo 'PRODUCT_MAINLINE_BLUETOOTH_SEPOLICY_DEV_CERTIFICATES=$(CERTIFICATE_COMMON)/data' >>"$PRODUCTMK"
